...
conn.commit()
c.execute(""" CREATE TABLE IF NOT EXISTS randevular2(
    isim TEXT,
    soyisim TEXT,
    model TEXT,
    plaka TEXT,
...
def randevuekle(isim,soyisim,model,plaka,tarih,fiyat,detay,telefon):
    conn = sqlite3.connect("serviscrm.db")
    c = conn.cursor()
    c.execute("INSERT INTO randevular2 VALUES(?,?,?,?,?,?,?,?)",(isim,soyisim,model,plaka,tarih,fiyat,detay,telefon))
    conn.commit()
...
def gununrandevu():
    ...
    c.execute("SELECT * FROM randevular2 WHERE tarih=? ",(bugun,))
    sonuc=c.fetchall()
